...
async fn download_video(api_key: &str, video_id: &str, output_dir: &str) -> Result<(), Box<dyn Error>> {
    let url = format!("https://www.googleapis.com/youtube/v3/videos?key={}&id={}&part=snippet", api_key, video_id);
    let client = Client::builder().build()?;
    let response = client
        .get(&url)
        .send()
